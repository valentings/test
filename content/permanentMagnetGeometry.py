...
#
    ...
    def __init__(self, d0 = 0.5e-3, Hm=2e-3, HFe=3e-3, Bm=4e-3, B=8e-3, maxh=1e-3, periodic=False, maxhEdges = 1):
        self.d0 = d0
        self.Hm = Hm
        self.HFe = HFe
        self.Bm = Bm
        ...
        points_xy = [(-B/2, -HFe), (B/2, -HFe), \
            (B/2, 0), (B/2, Hm+d0), (-B/2, Hm+d0), \
            (-B/2, 0), (-Bm/2, 0), (Bm/2, 0), (Bm/2, Hm), (-Bm/2, Hm)]
        

        wp = WorkPlane()
        wp.MoveTo(-B/2,-HFe)
        rotor = wp.Rectangle(B,HFe).Face()
        rotor.name = "rotor"
        ...
        wp.MoveTo(-Bm/2,0)
        magnet = wp.Rectangle(Bm,Hm).Face()
        magnet.name = "magnet"
        ...
        if periodic:
            ...
            edge_air_left = self.geo.edges.Nearest(Pnt(-B/2, Hm/2 , 0))
            edge_rotor_left = self.geo.edges.Nearest(Pnt(-B/2, -HFe/2 , 0))

            edge_air_right = self.geo.edges.Nearest(Pnt(B/2, Hm/2 , 0))
            edge_rotor_right = self.geo.edges.Nearest(Pnt(B/2, -HFe/2 , 0))
